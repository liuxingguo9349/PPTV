...
with tf.Session(config=config) as sess:
    ...
    reader = tf.train.NewCheckpointReader(checkpoint_path)  #读取checkpoint文件中保存的所有变量
    ...
    for key in var_to_shape_map:
        ...
        this_variables = tf.trainable_variables()
        for t in this_variables:

            if sourcename == t.name:
                sess.run(tf.assign(t, reader.get_tensor(key)))
                print(sourcename, 'assign to ', t.name)
                sourceweigrht = reader.get_tensor(key)
                thisweight = sess.run(t.name)
        # w4weight = sess.run(Variable_6:0)
